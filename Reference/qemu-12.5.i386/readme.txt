Just run the enclosed linux.bat and it should start up to the boot loader, WiniBoot.

From there either wait 30 seconds, or press 1, and the shoelace loader will then load
the Linux kernel.

From there you'll get dumped into a shell.

Linux at this point didn't have multi-user support, although it was multiprogram.

There is NO reboot, or shutdown program for linux at this point. You must
issue the sync command several times, then close Qemu.  Disk corruption is almost
unavoidable, worst case, just unzip this again.

This was all taken from oldlinux.org, and converted into the qcow2 format for
Qemu 0.12.5 (the current version).
...
2006.6.25
        Input the 'Linux 0.00' package into the harddisk image file hdc-0.11-ne
.img
...
2005.5.18
        ...
        Bochs-2.2.X.i586.rpm -- Bochs system for use in Linux system environmen
.
                             You can ALWAYS download the newest version from
                             http://sourceforge.net/projects/bochs/
        bootimage-0.11    -- kernel 0.11 bootimage request rootimage from flopp in a:.
        ...
        hdc-0.11-new.img  -- harddisk root filesystem image which contains bootcode.
        diskb.img         -- a dos format disk image. can be w/r with mtools.
                             example: mdir b:, mcopy b:, mdel b:filename
        bochsrc-fda.bxrc  -- bochs rc file configured for request to use disk rot iamge.
        bochsrc-fdb.bxrc  -- Configured for using disk root image in fd1.
        bochsrc-hd.bxrc   -- Configured for using hd root image and boot from f0.
        bochsrc-hdboot.bxrc -- Configured for using hd root image and boot from hd.
                             Notice that the boot Image is located at /usr/src/inux/Image.
...
2004.3.29
        ...
        winimage program or something like that to add/delete files in diskb.im
